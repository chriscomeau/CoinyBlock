                    ...
                    if ([self previousGameCenterAvailability] != GameCenterAvailabilityNoPlayer) {
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            ...
                            if ([[self delegate] respondsToSelector:@selector(gameCenterManager:authenticateUser:)]) {
                                ...
                                NSLog(@"GameCenterManager: [ERROR] %@ Fails to Respond to the required delegate method gameCenterManager:authenticateUser:. This delegate method must be properly implemented to use GC Manager", [self delegate]);
                            }
                        });
                    }
                } else if (!error) {
